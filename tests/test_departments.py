...
from brain.tools import _DEMO_SLOTS, ToolOutcome, build_default_registry, resolve_department
...
@pytest.mark.parametrize("said", ["pediatrics", "surgery", "oncology", "", "   ", "asdf"])
...
def test_every_alias_points_at_a_real_department():
    from brain.tools import _DEPARTMENT_ALIASES

    for alias, target in _DEPARTMENT_ALIASES.items():
        assert target in _DEMO_SLOTS, f"{alias} -> {target} is not a department"
...
async def test_an_unknown_department_yields_no_slots():
    ...
    outcome = await build_default_registry().invoke(
        "check_availability", {"department": "pediatrics"}
    )
...
async def test_the_caller_is_offered_the_real_list_instead():
    ...
    assert outcome.result["known_departments"] == sorted(_DEMO_SLOTS)
...
async def test_a_recognised_alias_returns_that_departments_own_slots():
    ...
    assert outcome.result["available_slots"] == _DEMO_SLOTS["ent"]
...
async def test_a_successful_booking_records_the_canonical_department():
    outcome = await build_default_registry().invoke(
        "book_appointment",
        {"patient_name": "Amit", "slot": "tomorrow 11:30 am", "phone": "9900000000",
         "department": "synus"},
    )
    assert outcome.result["booked"] is True
    assert outcome.result["department"] == "ent"
